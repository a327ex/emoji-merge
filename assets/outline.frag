vec4 effect(vec4 vcolor, Image texture, vec2 tc, vec2 pc) {
  ...
  float a = 0.0;
  a += Texel(texture, vec2(tc.x - 2*x, tc.y - 2*y)).a;
  a += Texel(texture, vec2(tc.x - x, tc.y - 2*y)).a;
  a += Texel(texture, vec2(tc.x, tc.y - 2*y)).a;
  a += Texel(texture, vec2(tc.x + x, tc.y - 2*y)).a;
  a += Texel(texture, vec2(tc.x + 2*x, tc.y - 2*y)).a;
  a += Texel(texture, vec2(tc.x - 2*x, tc.y - y)).a;
  a += Texel(texture, vec2(tc.x - x, tc.y - y)).a;
  a += Texel(texture, vec2(tc.x, tc.y - y)).a;
  a += Texel(texture, vec2(tc.x + x, tc.y - y)).a;
  a += Texel(texture, vec2(tc.x + 2*x, tc.y - y)).a;
  a += Texel(texture, vec2(tc.x - 2*x, tc.y)).a;

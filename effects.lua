emoji_merge_effect = function(x, y, args)
  local self = anchor('emoji_merge_effect', args)
  self:prs_init(x, y)
  self:hitfx_init()
  ...
  self.update = function(self, dt) game2:draw_image(self.emoji, self.x, self.y, self.r, self.sx*self.springs.main.x, self.sy*self.springs.main.x, nil, nil, colors.white[0], self.flashes.main.x and shaders.combine) end
  return self
end
...
hit_effect = function(x, y, args)
  ...
  self.update = function(self, dt) self.animation:animation_update(dt, effects, self.x, self.y, self.r, self.sx, self.sy) end
  return self
end
